window.cpexWebsiteSettings = {
  website: {
    name: 'aktualne.cz',
    note: 'integrace: FED-497'
  },
  headerbidding: {
    adUnits: [{
      code: 'leader',
      mediaTypes: {
        banner: {
          sizes: [
            [728, 90],
            [970, 100],
            [970, 210],
            [2000, 1400]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 353314,
          zoneId: 2613988,
          accountId: '10900',
          position: 'atf',
          sizes: [2, 95, 96, 230]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 353320,
          zoneId: 2186652,
          accountId: '10900',
          position: 'atf',
          sizes: [2, 95, 96, 230]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic_eco_hb',
        params: {
          publisherId: '161532',
          adSlot: '4344927'
        },
        bidderModuleName: 'pubmatic'
      }, {
        bidder: 'knowlimits-adform',
        params: {
          mid: 1901536,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'adform_im_hb',
        params: {
          mid: 1334461,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'adform_im_hb-mask',
        params: {
          mid: 1334462,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'adform',
        params: {
          mid: 747148,
          rcur: 'USD'
        }
      }, {
        bidder: 'ix',
        params: {
          siteId: 759650
        }
      }, {
        bidder: 'teads',
        params: {
          placementId: 211489,
          pageId: 195954
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 28537849
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172,
          uid: 'eco_banner_low'
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'ECO_aktualnecz_leader'
        }
      }, {
        bidder: 'r2b2',
        params: {
          d: 'aktualne.cz',
          p: '728x90_2',
          m: 0,
          g: 'hb'
        }
      }]
    }, {
      code: 'halfpage-1',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 600],
            [300, 250],
            [300, 300],
            [160, 600]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 353314,
          zoneId: 2186670,
          accountId: '10900',
          position: 'atf',
          sizes: [10, 15, 48, 9]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 353320,
          zoneId: 2186656,
          accountId: '10900',
          position: 'atf',
          sizes: [10, 15, 48, 9]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic_eco_hb',
        params: {
          publisherId: '161532',
          adSlot: '4344925'
        },
        bidderModuleName: 'pubmatic'
      }, {
        bidder: 'knowlimits-adform',
        params: {
          mid: 1840838,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'adform_im_hb',
        params: {
          mid: 1334463,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'adform_im_hb-mask',
        params: {
          mid: 1334464,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'omg-adform',
        params: {
          mid: 1869676,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'adform',
        params: {
          mid: 1109020,
          rcur: 'USD'
        }
      }, {
        bidder: 'ix',
        params: {
          siteId: 759648
        }
      }, {
        bidder: 'teads',
        params: {
          placementId: 211497,
          pageId: 195962
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 23941422
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172,
          uid: 'eco_banner_low'
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'ECO_aktualnecz_halfpage_1'
        }
      }, {
        bidder: 'r2b2',
        params: {
          d: 'aktualne.cz',
          p: '300x600_1',
          m: 0,
          g: 'hb'
        }
      }]
    }, {
      code: 'halfpage-2',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 600],
            [300, 250],
            [300, 300],
            [160, 600]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 353314,
          zoneId: 2186672,
          accountId: '10900',
          position: 'atf',
          sizes: [10, 15, 48, 9]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 353320,
          zoneId: 2186658,
          accountId: '10900',
          position: 'atf',
          sizes: [10, 15, 48, 9]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic_eco_hb',
        params: {
          publisherId: '161532',
          adSlot: '4344926'
        },
        bidderModuleName: 'pubmatic'
      }, {
        bidder: 'knowlimits-adform',
        params: {
          mid: 1840839,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'adform_im_hb',
        params: {
          mid: 1334465,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'adform_im_hb-mask',
        params: {
          mid: 1334466,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'omg-adform',
        params: {
          mid: 1869689,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'adform',
        params: {
          mid: 1109049,
          rcur: 'USD'
        }
      }, {
        bidder: 'ix',
        params: {
          siteId: 759649
        }
      }, {
        bidder: 'teads',
        params: {
          placementId: 211490,
          pageId: 195955
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 23941427
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172,
          uid: 'eco_banner_low'
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'ECO_aktualnecz_halfpage_2'
        }
      }, {
        bidder: 'r2b2',
        params: {
          d: 'aktualne.cz',
          p: '300x600_2',
          m: 0,
          g: 'hb'
        }
      }]
    }, {
      code: 'wallpaper',
      mediaTypes: {
        banner: {
          sizes: [
            [480, 300],
            [300, 300],
            [300, 250],
            [336, 280]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 353314,
          zoneId: 2186674,
          accountId: '10900',
          position: 'atf',
          sizes: [83, 48, 15, 16]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 353320,
          zoneId: 2186660,
          accountId: '10900',
          position: 'atf',
          sizes: [83, 48, 15, 16]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic_eco_hb',
        params: {
          publisherId: '161532',
          adSlot: '4344929'
        },
        bidderModuleName: 'pubmatic'
      }, {
        bidder: 'knowlimits-adform',
        params: {
          mid: 1840840,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'adform_im_hb',
        params: {
          mid: 1334467,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'adform_im_hb-mask',
        params: {
          mid: 1334468,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'omg-adform',
        params: {
          mid: 1869702,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'adform',
        params: {
          mid: 1109030,
          rcur: 'USD'
        }
      }, {
        bidder: 'ix',
        params: {
          siteId: 759656
        }
      }, {
        bidder: 'teads',
        params: {
          placementId: 211500,
          pageId: 195965
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 23941423
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172,
          uid: 'eco_banner_low'
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'ECO_aktualnecz_wallpaper'
        }
      }, {
        bidder: 'r2b2',
        params: {
          d: 'aktualne.cz',
          p: '480x300',
          m: 0,
          g: 'hb'
        }
      }]
    }, {
      code: 'megaboard',
      mediaTypes: {
        banner: {
          sizes: [
            [970, 310],
            [970, 250],
            [970, 210],
            [728, 90]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 353314,
          zoneId: 2186668,
          accountId: '10900',
          position: 'atf',
          sizes: [94, 57, 96, 2]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 353320,
          zoneId: 2186654,
          accountId: '10900',
          position: 'atf',
          sizes: [94, 57, 96, 2]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic_eco_hb',
        params: {
          publisherId: '161532',
          adSlot: '4344928'
        },
        bidderModuleName: 'pubmatic'
      }, {
        bidder: 'knowlimits-adform',
        params: {
          mid: 1840841,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'adform_im_hb',
        params: {
          mid: 1334469,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'adform_im_hb-mask',
        params: {
          mid: 1334476,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'omg-adform',
        params: {
          mid: 1869759,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'adform',
        params: {
          mid: 1109045,
          rcur: 'USD'
        }
      }, {
        bidder: 'ix',
        params: {
          siteId: 759654
        }
      }, {
        bidder: 'teads',
        params: {
          placementId: 211492,
          pageId: 195957
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 23941421
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172,
          uid: 'eco_banner_low'
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'ECO_aktualnecz_megaboard'
        }
      }, {
        bidder: 'r2b2',
        params: {
          d: 'aktualne.cz',
          p: '970x210_2',
          m: 0,
          g: 'hb'
        }
      }]
    }, {
      code: 'mediumrectangle-mob-pos1',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 300],
            [300, 250],
            [250, 250],
            [480, 820],
            [480, 480],
            [336, 280]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 353316,
          zoneId: 2186692,
          accountId: '10900',
          position: 'atf',
          sizes: [48, 15, 14, 256, 261, 16]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 353318,
          zoneId: 2186682,
          accountId: '10900',
          position: 'atf',
          sizes: [48, 15, 14, 256, 261, 16]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic_eco_hb',
        params: {
          publisherId: '161532',
          adSlot: '4344931'
        },
        bidderModuleName: 'pubmatic'
      }, {
        bidder: 'knowlimits-adform',
        params: {
          mid: 1840842,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'omg-adform',
        params: {
          mid: 1869772,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'adform',
        params: {
          mid: 1109053,
          rcur: 'USD'
        }
      }, {
        bidder: 'ix',
        params: {
          siteId: 759651
        }
      }, {
        bidder: 'teads',
        params: {
          placementId: 211496,
          pageId: 195961
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 28672132
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172,
          uid: 'eco_banner_low'
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'ECO_aktualnecz_mediumrectangle_mob_pos1'
        }
      }, {
        bidder: 'r2b2',
        params: {
          d: 'aktualne.cz',
          p: '300x250_pos1',
          m: 1,
          g: 'hb'
        }
      }]
    }, {
      code: 'mediumrectangle-mob-pos2',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 300],
            [300, 250],
            [250, 250],
            [480, 480],
            [336, 280]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 353316,
          zoneId: 2186694,
          accountId: '10900',
          position: 'atf',
          sizes: [48, 15, 14, 261, 16]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 353318,
          zoneId: 2186684,
          accountId: '10900',
          position: 'atf',
          sizes: [48, 15, 14, 261, 16]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic_eco_hb',
        params: {
          publisherId: '161532',
          adSlot: '4344932'
        },
        bidderModuleName: 'pubmatic'
      }, {
        bidder: 'knowlimits-adform',
        params: {
          mid: 1840843,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'omg-adform',
        params: {
          mid: 1869775,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'adform',
        params: {
          mid: 1109065,
          rcur: 'USD'
        }
      }, {
        bidder: 'ix',
        params: {
          siteId: 759652
        }
      }, {
        bidder: 'teads',
        params: {
          placementId: 211488,
          pageId: 195953
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 23941418
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172,
          uid: 'eco_banner_low'
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'ECO_aktualnecz_mediumrectangle_mob_pos2'
        }
      }, {
        bidder: 'r2b2',
        params: {
          d: 'aktualne.cz',
          p: '300x250_pos2',
          m: 1,
          g: 'hb'
        }
      }]
    }, {
      code: 'mediumrectangle-mob-pos3',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 300],
            [300, 250],
            [250, 250],
            [480, 480],
            [336, 280]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 353316,
          zoneId: 2186696,
          accountId: '10900',
          position: 'atf',
          sizes: [48, 15, 14, 261, 16]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 353318,
          zoneId: 2186686,
          accountId: '10900',
          position: 'atf',
          sizes: [48, 15, 14, 261, 16]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic_eco_hb',
        params: {
          publisherId: '161532',
          adSlot: '4344933'
        },
        bidderModuleName: 'pubmatic'
      }, {
        bidder: 'knowlimits-adform',
        params: {
          mid: 1840844,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'omg-adform',
        params: {
          mid: 1869777,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'adform',
        params: {
          mid: 1109071,
          rcur: 'USD'
        }
      }, {
        bidder: 'ix',
        params: {
          siteId: 759653
        }
      }, {
        bidder: 'teads',
        params: {
          placementId: 211491,
          pageId: 195956
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 23941419
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172,
          uid: 'eco_banner_low'
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'ECO_aktualnecz_mediumrectangle_mob_pos3'
        }
      }, {
        bidder: 'r2b2',
        params: {
          d: 'aktualne.cz',
          p: '300x250_pos3',
          m: 1,
          g: 'hb'
        }
      }]
    }, {
      code: 'popup',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 250],
            [500, 200]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 353316,
          zoneId: 2632554,
          accountId: '10900',
          position: 'atf',
          sizes: [15, 258]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 353318,
          zoneId: 2632552,
          accountId: '10900',
          position: 'atf',
          sizes: [15, 258]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'knowlimits-adform',
        params: {
          mid: 1840845,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'omg-adform',
        params: {
          mid: 1869781,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 27917907
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172,
          uid: 'eco_banner_low'
        }
      }]
    }, {
      code: 'strip',
      mediaTypes: {
        banner: {
          sizes: [
            [728, 90],
            [320, 50],
            [300, 50]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 353316,
          zoneId: 2186688,
          accountId: '10900',
          position: 'atf',
          sizes: [2, 43, 44]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 353318,
          zoneId: 2186678,
          accountId: '10900',
          position: 'atf',
          sizes: [2, 43, 44]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic_eco_hb',
        params: {
          publisherId: '161532',
          adSlot: '4344930'
        },
        bidderModuleName: 'pubmatic'
      }, {
        bidder: 'adform',
        params: {
          mid: 1109051,
          rcur: 'USD'
        }
      }, {
        bidder: 'ix',
        params: {
          siteId: 759651
        }
      }, {
        bidder: 'teads',
        params: {
          placementId: 211495,
          pageId: 195960
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 23941416
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172,
          uid: 'eco_banner_low'
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'ECO_aktualnecz_strip'
        }
      }, {
        bidder: 'r2b2',
        params: {
          d: 'aktualne.cz',
          p: '320x50',
          m: 1,
          g: 'hb'
        }
      }]
    }],
    currency: 'USD'
  },
  formats: {
    skin: {
      enabled: true,
      contentEl: /*S*/ document.getElementsByClassName('page')[0] /*E*/,
      offsetScroll: true,
      hide: [/*S*/document.getElementsByClassName('advert__leader')[0]/*E*/],
      generalCSS: "document.querySelector('.advert__leader').style.minHeight = '0';"
    },
    interscroller: {
      enabled: true,
      adUnits: ['sas-injected-1'],
      offset: 54
    },
    slideup: {
      adUnits: ['sas-special4'],
      enabled: true,
      backgroundEl: /*S*/ document.getElementById('reklama-special4') /*E*/
    },
    responsive: {
      width: 480,
      height: 480,
      adUnits: ['sas-injected-1', 'sas-injected-2', 'sas-injected-3'],
      enabled: true
    }
  },
  general: {
    errorPath: 'https://73f2bd72d0d2477ab2f976d6098fe246@o530000.ingest.sentry.io/4504531846365184',
    beforeLoad: /*S*/async () => {
  return new Promise((resolve) => {
    /* applies only if pay or ok is present */
    if (!window.cpexCmpSubscription) {
      /* once didomi loads, disable hb if no consent found for purpose 1 or 2 */
      window.didomiOnReady = window.didomiOnReady || [];
      window.didomiOnReady.push(function (Didomi) {
        const consent = Didomi.getCurrentUserStatus();
        if (
          !consent.purposes.cookies.enabled ||
          !consent.purposes.select_basic_ads.enabled
        ) {
          cpexPackage.utils.cpexWarn(
            'No consent for purpose 1 or 2, disabling HB'
          );
          cpexPackage.settings.headerbidding.enabled = false;
        }
        resolve();
      });
      setTimeout(() => {
        /* fallback */
        cpexPackage.settings.headerbidding.enabled = false;
        resolve();
      }, 1000);
    }
  });
};/*E*/
  }
}