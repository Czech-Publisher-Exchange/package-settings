window.cpexWebsiteSettings = {
  cmp: {
    enabled: false
  },
  aam: {
    settings: {
      publisher: 'vlm'
    }
  },
  headerbidding: {
    enabled: true,
    adUnits: [{
      code: 'leaderboard-top',
      mediaTypes: {
        banner: {
          sizes: [
            [728, 90],
            [970, 210],
            [2000, 1400],
            [970, 250]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 26460,
          zoneId: 1624548,
          accountId: '10900',
          position: 'atf',
          sizes: [2, 96, 230, 57]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 25692,
          zoneId: 553014,
          accountId: '10900',
          position: 'atf',
          sizes: [2, 96, 230, 57]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4700226'
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 15783923
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'VLM_dotyk_leaderboard-top'
        }
      }],
      filter: {
        minWidth: 1000
      }
    }, {
      code: 'square-1',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 250],
            [300, 300]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 26460,
          zoneId: 503834,
          accountId: '10900',
          position: 'atf',
          sizes: [15, 48]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 25692,
          zoneId: 540542,
          accountId: '10900',
          position: 'atf',
          sizes: [15, 48]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4700203'
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 15783926
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'VLM_dotyk_square-1'
        }
      }],
      filter: {
        minWidth: 1000
      }
    }, {
      code: 'leaderboard-bottom',
      mediaTypes: {
        banner: {
          sizes: [
            [728, 90],
            [970, 250],
            [970, 210]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 26460,
          zoneId: 503856,
          accountId: '10900',
          position: 'atf',
          sizes: [2, 57, 96]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 25692,
          zoneId: 1501586,
          accountId: '10900',
          position: 'atf',
          sizes: [2, 57, 96]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4700213'
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 15783924
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'VLM_dotyk_leaderboard-bottom'
        }
      }],
      filter: {
        minWidth: 1000
      }
    }, {
      code: 'skyscraper-1',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 600],
            [300, 250],
            [160, 600]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 26460,
          zoneId: 503842,
          accountId: '10900',
          position: 'atf',
          sizes: [10, 15, 9]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 25692,
          zoneId: 2045620,
          accountId: '10900',
          position: 'atf',
          sizes: [10, 15, 9]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4700211'
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 15783925
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'VLM_dotyk_skyscraper-1'
        }
      }],
      filter: {
        minWidth: 1000
      }
    }, {
      code: 'm-square-1',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 250],
            [300, 300],
            [480, 820]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 46270,
          zoneId: 1650472,
          accountId: '10900',
          position: 'atf',
          sizes: [15, 48, 256]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 46272,
          zoneId: 1650468,
          accountId: '10900',
          position: 'atf',
          sizes: [15, 48, 256]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4700200'
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 18980271
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'VLM_dotyk_m-square-1'
        }
      }],
      filter: {
        maxWidth: 1000
      }
    }, {
      code: 'm-square-2',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 250],
            [300, 300]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 46270,
          zoneId: 1650474,
          accountId: '10900',
          position: 'atf',
          sizes: [15, 48]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 46272,
          zoneId: 1650470,
          accountId: '10900',
          position: 'atf',
          sizes: [15, 48]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4700201'
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 18980273
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'VLM_dotyk_m-square-2'
        }
      }],
      filter: {
        maxWidth: 1000
      }
    }, {
      code: 'm-sticky',
      mediaTypes: {
        banner: {
          sizes: [
            [320, 150],
            [300, 250]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 46270,
          zoneId: 1077194,
          accountId: '10900',
          position: 'atf',
          sizes: [60, 15]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 46272,
          zoneId: 1077196,
          accountId: '10900',
          position: 'atf',
          sizes: [60, 15]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 22969029
        }
      }],
      filter: {
        maxWidth: 1000
      }
    }, {
      code: 'commercial-article-2',
      mediaTypes: {
        banner: {
          sizes: [
            [480, 300],
            [300, 300],
            [300, 250]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 26460,
          zoneId: 1340958,
          accountId: '10900',
          position: 'atf',
          sizes: [83, 48, 15]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 25692,
          zoneId: 1340960,
          accountId: '10900',
          position: 'atf',
          sizes: [83, 48, 15]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4700212'
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 16370636
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }],
      filter: {
        minWidth: 1000
      }
    }, {
      code: 'commercial-article-native-detail',
      mediaTypes: {
        banner: {
          sizes: [
            [548, 150]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 144588,
          zoneId: 1370788,
          accountId: '10900',
          position: 'atf',
          sizes: [90]
        }
      }],
      filter: {
        minWidth: 1000
      }
    }, {
      code: 'commercial-article-2-GA-hp',
      mediaTypes: {
        banner: {
          sizes: [
            [548, 150]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 144588,
          zoneId: 2642272,
          accountId: '10900',
          position: 'atf',
          sizes: [90]
        }
      }],
      filter: {
        minWidth: 1000
      }
    }, {
      code: 'm-commercial-article-native-detail',
      mediaTypes: {
        banner: {
          sizes: [
            [548, 150]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 46270,
          zoneId: 2642278,
          accountId: '10900',
          position: 'atf',
          sizes: [90]
        }
      }],
      filter: {
        maxWidth: 1000
      }
    }, {
      code: 'skyscraper-3',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 600],
            [300, 250],
            [300, 300]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 26460,
          zoneId: 2587714,
          accountId: '10900',
          position: 'atf',
          sizes: [10, 15, 48]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 25692,
          zoneId: 2588678,
          accountId: '10900',
          position: 'atf',
          sizes: [10, 15, 48]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4719752'
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 27587624
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'VLM_dotykcz_Skyscraper_3_300x600'
        }
      }],
      filter: {
        minWidth: 1000
      }
    }, {
      code: 'skyscraper-4',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 600],
            [300, 250],
            [300, 300]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 26460,
          zoneId: 2587716,
          accountId: '10900',
          position: 'atf',
          sizes: [10, 15, 48]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 25692,
          zoneId: 2588680,
          accountId: '10900',
          position: 'atf',
          sizes: [10, 15, 48]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4719751'
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 27587705
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'VLM_dotykcz_skyscraper_4_300x600'
        }
      }],
      filter: {
        minWidth: 1000
      }
    }, {
      code: 'leaderboard-bottom-2',
      mediaTypes: {
        banner: {
          sizes: [
            [728, 90],
            [970, 90],
            [970, 210],
            [970, 250]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 26460,
          zoneId: 2587718,
          accountId: '10900',
          position: 'atf',
          sizes: [2, 55, 96, 57]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 25692,
          zoneId: 2588682,
          accountId: '10900',
          position: 'atf',
          sizes: [2, 55, 96, 57]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4719748'
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 27587726
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'VLM_dotykcz_Leaderboard_bottom_2_970x250'
        }
      }],
      filter: {
        minWidth: 1000
      }
    }, {
      code: 'm-square-3',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 300],
            [300, 250]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 46270,
          zoneId: 2587720,
          accountId: '10900',
          position: 'atf',
          sizes: [48, 15]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 46272,
          zoneId: 2588684,
          accountId: '10900',
          position: 'atf',
          sizes: [48, 15]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4719750'
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 27587733
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'VLM_dotykcz_m_square_3_300x300'
        }
      }],
      filter: {
        maxWidth: 1000
      }
    }, {
      code: 'm-square-4',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 250],
            [300, 300],
            [480, 480],
            [480, 300]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 46270,
          zoneId: 2587722,
          accountId: '10900',
          position: 'atf',
          sizes: [15, 48, 261, 83]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 46272,
          zoneId: 2588686,
          accountId: '10900',
          position: 'atf',
          sizes: [15, 48, 261, 83]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4719749'
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 27587739
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'VLM_dotykcz_m_square_4_480x480'
        }
      }],
      filter: {
        maxWidth: 1000
      }
    }, {
      code: 'm-square-5',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 250],
            [300, 300],
            [480, 480],
            [480, 300]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 46270,
          zoneId: 2587740,
          accountId: '10900',
          position: 'atf',
          sizes: [15, 48, 261, 83]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 46272,
          zoneId: 2588722,
          accountId: '10900',
          position: 'atf',
          sizes: [15, 48, 261, 83]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4719747'
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 27587745
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'VLM_dotykcz_m_square_5_480x480'
        }
      }],
      filter: {
        maxWidth: 1000
      }
    }, {
      code: 'm-square-bottom-1',
      mediaTypes: {
        banner: {
          sizes: [
            [336, 280],
            [300, 250],
            [300, 300],
            [480, 480],
            [480, 300]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 46270,
          zoneId: 2614516,
          accountId: '10900',
          position: 'atf',
          sizes: [16, 15, 48, 261, 83]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 46272,
          zoneId: 2614524,
          accountId: '10900',
          position: 'atf',
          sizes: [16, 15, 48, 261, 83]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4746472'
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 27739530
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'VLM_dotykcz_m_square_bottom_1_300x250'
        }
      }],
      filter: {
        maxWidth: 1000
      }
    }, {
      code: 'm-square-bottom-2',
      mediaTypes: {
        banner: {
          sizes: [
            [336, 280],
            [300, 250],
            [300, 300],
            [480, 480],
            [480, 300]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 46270,
          zoneId: 2614518,
          accountId: '10900',
          position: 'atf',
          sizes: [16, 15, 48, 261, 83]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 46272,
          zoneId: 2614526,
          accountId: '10900',
          position: 'atf',
          sizes: [16, 15, 48, 261, 83]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4746475'
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 27739547
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'VLM_dotykcz_m_square_bottom_2_300x250'
        }
      }],
      filter: {
        maxWidth: 1000
      }
    }, {
      code: 'm-square-bottom-3',
      mediaTypes: {
        banner: {
          sizes: [
            [336, 280],
            [300, 250],
            [300, 300],
            [480, 480],
            [480, 300]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 46270,
          zoneId: 2614522,
          accountId: '10900',
          position: 'atf',
          sizes: [16, 15, 48, 261, 83]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 46272,
          zoneId: 2614530,
          accountId: '10900',
          position: 'atf',
          sizes: [16, 15, 48, 261, 83]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4746476'
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 27739763
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'VLM_dotykcz_m_square_bottom_3_300x250'
        }
      }],
      filter: {
        maxWidth: 1000
      }
    }, {
      code: 'm-square-gallery-1',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 300],
            [300, 250],
            [336, 280],
            [480, 820]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 46270,
          zoneId: 2681990,
          accountId: '10900',
          position: 'atf',
          sizes: [48, 15, 16, 256]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 46272,
          zoneId: 2681996,
          accountId: '10900',
          position: 'atf',
          sizes: [48, 15, 16, 256]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4843351'
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 28401650
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'VLM_dotykcz_m_square_gallery_1'
        }
      }],
      filter: {
        maxWidth: 1000
      }
    }, {
      code: 'm-square-gallery-2',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 300],
            [300, 250],
            [336, 280],
            [480, 480]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 46270,
          zoneId: 2681992,
          accountId: '10900',
          position: 'atf',
          sizes: [48, 15, 16, 261]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 46272,
          zoneId: 2681998,
          accountId: '10900',
          position: 'atf',
          sizes: [48, 15, 16, 261]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4843352'
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 28401651
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'VLM_dotykcz_m_square_gallery_2'
        }
      }],
      filter: {
        maxWidth: 1000
      }
    }, {
      code: 'm-square-gallery-3',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 300],
            [300, 250],
            [336, 280],
            [480, 480]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 46270,
          zoneId: 2681994,
          accountId: '10900',
          position: 'atf',
          sizes: [48, 15, 16, 261]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 46272,
          zoneId: 2682000,
          accountId: '10900',
          position: 'atf',
          sizes: [48, 15, 16, 261]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4843353'
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 28401659
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'triplelift',
        params: {
          inventoryCode: 'VLM_dotykcz_m_square_gallery_3'
        }
      }],
      filter: {
        maxWidth: 1000
      }
    }]
  },
  website: {
    name: 'dotyk.cz'
  },
  formats: {
    skin: {
      enabled: true,
      contentEl: (document.getElementsByClassName('page')[0]),
      hide: [(document.getElementsByClassName('reklama-box-leaderboard-top')[0])],
      offset: 32,
      offsetScroll: true
    },
    interscroller: {
      enabled: true
    },
    native: {
      enabled: true,
      templates: {
        'commercial-article-native-detail': "<div class='clanek-item' style='padding-left: 2%; width: 100%'>\n  <a class='dom-image-wrap' href='${link}' target='_blank' rel='sponsored noopener'>\n    <img width='200' height='113' loading='lazy' alt='Reklama' class='dom-image' src='${img}' style='height: 113px; object-fit: cover'>\n  </a>\n  <div class='text-wrapper' style='height: 100px'>\n    <a class='rubrika-title-flex'>\n      <h3 class='text-doporucene tema rubrika-title'>\n        <span>Reklama</span>\n      </h3>\n    </a>\n    <a href='${link}' target='_blank' class='clanek-title-flex' rel='sponsored noopener'>\n      <h3 class='text-doporucene title clanek-title' style='margin-top: 10px'>\n        <span>${title}</span>\n      </h3>\n    </a>\n    <a href='${link}' class='clanek-popis-flex' title='${desc}'>\n      <h3 class='text-doporucene clanek-popis'>\n        <span>${desc}</span>\n      </h3>\n    </a>\n  </div>\n</div>",
        'commercial-article-1-hp': "<div class='article-box-a'>\n  <a class='article-box-a__img' target='_blank' tabindex='-1' href='${link}' rel='sponsored noopener'>\n    <img src='${img}' width='220' height='220' alt='Reklama' title='${title}' loading='lazy' style='object-fit: cover'>\n  </a>\n  <div class='article-box-a__inner'>\n    <div class='article-box-a__tag'>\n      <a class='article-box-a__tag-item'>Reklama</a>\n    </div>\n    <a class='ac-clanek' href='${link}' target='_blank' rel='sponsored noopener'>\n      <h2 class='article-box-a__title'>${title}</h2>\n      <p class='article-box-a__description'>${desc}</p>\n    </a>\n  </div>\n</div>",
        'commercial-article-2-hp': "<div class='article-box-a'>\n  <a class='article-box-a__img' target='_blank' tabindex='-1' href='${link}' target='_blank' rel='sponsored noopener'>\n    <img src='${img}' width='220' height='220' alt='Reklama' title='${title}' loading='lazy' style='object-fit: cover'>\n  </a>\n  <div class='article-box-a__inner'>\n    <div class='article-box-a__tag'>\n      <a class='article-box-a__tag-item'>Reklama</a>\n    </div>\n    <a class='ac-clanek' href='${link}' target='_blank' rel='sponsored noopener'>\n      <h2 class='article-box-a__title'>${title}</h2>\n      <p class='article-box-a__description'>${desc}</p>\n    </a>\n  </div>\n</div>",
        'm-commercial-article-1': "<div class='article-box-a'>\n  <div class='article-box-a__tag'>\n    <span class='article-box-a__tag-item'>REKLAMA</span>\n  </div>\n  <a href='${link}' title='${title}' tabindex='-1' target='_blank' rel='sponsored noopener'>\n    <img src='${img}' width='400' height='225' class='article-box-a__img' sizes='100vw' loading='lazy'>\n  </a>\n  <div class='article-box-a__inner'>\n    <a href='${link}' target='_blank' rel='sponsored noopener'><h2 class='article-box-a__title'>${title}</h2></a>\n    <div class='article-box-a__date-box'>${desc}</div>\n  </div>\n</div>",
        'm-commercial-article-2': "<div class='article-box-a'>\n  <div class='article-box-a__tag'>\n    <span class='article-box-a__tag-item'>REKLAMA</span>\n  </div>\n  <a href='${link}' target='_blank' title='${title}' tabindex='-1' rel='sponsored noopener'>\n    <img src='${img}' width='400' height='225' class='article-box-a__img' sizes='100vw' loading='lazy'>\n  </a>\n  <div class='article-box-a__inner'>\n    <a href='${link}' target='_blank' rel='sponsored noopener'><h2 class='article-box-a__title'>${title}</h2></a>\n    <div class='article-box-a__date-box'>${desc}</div>\n  </div>\n</div>",
        'm-commercial-article-native-detail': "<div class='container'>\n  <div class='clanek-item'>\n    <a href='${link}' target='_blank' rel='sponsored noopener'>\n      <img loading='lazy' alt='Reklama' width='130' height='73' src='${img}' class='dom-image'>\n      <div class='text-wrapper'>\n        <h3 class='text-doporucene title clanek-title'>\n          <span>${title} ${desc}</span>\n        </h3>\n      </div>\n    </a>\n  </div>\n</div>",
        'commercial-article-2-GA-hp': "<style>\n  .cpex-native a { height: 300px; display: flex; text-decoration: none; color: inherit; align-items: center }\n  .cpex-native img { width: 300px; object-fit: cover }\n  .cpex-native .text { padding-left: 20px }\n  .cpex-native .text h2 { font-size: 20pt }\n</style>\n<div class='cpex-native'>\n  <a href='${link}' target='_blank' rel='sponsored noopener'>\n    <img src='${img}'>\n    <div class='text'>\n      <div class='article-box-a__tag' style='height: 10px'>\n        <span class='article-box-a__tag-item'>Reklama</span>\n      </div>\n      <h2>${title}</h2>\n      <p>${desc}</p>\n    </div>\n  </a>\n</div>",
        'commercial-article-2-GA': "<style>\n  .cpex-native a { display: flex; text-decoration: none; color: inherit; align-items: center }\n  .cpex-native img { object-fit: cover; min-width: 16rem }\n</style>\n<div class='cpex-native'>\n  <a href='${link}' target='_blank' rel='sponsored noopener' class='article-list__item article-list-item article-list-item--default'>\n    <img src='${img}'>\n    <div class='article-list-item__headline'>\n      <div class='article-box-a__tag'>\n        <span class='article-list-item__rubric'>Reklama</span>\n      </div>\n      <h2 class='article-list-item__title'>${title}</h2>\n      <p class='article-list-item__perex'>${desc}</p>\n    </div>\n  </a>\n</div>"
      }
    },
    slideup: {
      enabled: true,
      adUnit: 'm-sticky'
    },
    vignette: {
      enabled: true,
      adUnit: 'm-vignette'
    }
  },
  adserver: {
    adapter: 'sastracker',
    areaKey: 'pos',
    allowedSSPs: {
      pubmatic: 1,
      index: 3,
      magnite: 4,
      xandr: 164
    }
  }
}