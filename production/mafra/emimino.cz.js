window.cpexWebsiteSettings = {
  website: {
    name: 'emimino.cz'
  },
  headerbidding: {
    adUnits: [{
      code: 'branding',
      mediaTypes: {
        banner: {
          sizes: [
            [998, 200],
            [728, 90],
            [970, 210],
            [970, 90],
            [2000, 1400]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 144582,
          zoneId: 731360,
          accountId: '10900',
          position: 'atf',
          sizes: [259, 2, 96, 55, 230]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 144584,
          zoneId: 731528,
          accountId: '10900',
          position: 'atf',
          sizes: [259, 2, 96, 55, 230]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4641172'
        }
      }, {
        bidder: 'adform',
        params: {
          mid: 1634956,
          rcur: 'USD'
        }
      }, {
        bidder: 'omg-adform',
        params: {
          mid: 1658702,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'fragile-adform',
        params: {
          mid: 1655727,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'ix',
        params: {
          siteId: 878556
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 27156773
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'r2b2',
        params: {
          d: 'emimino.cz',
          p: 'branding',
          m: 0,
          g: 'hb'
        }
      }]
    }, {
      code: 'halfpagead',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 600],
            [300, 300],
            [300, 250],
            [160, 600],
            [250, 250]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 144582,
          zoneId: 731348,
          accountId: '10900',
          position: 'atf',
          sizes: [10, 48, 15, 9, 14]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 144584,
          zoneId: 731522,
          accountId: '10900',
          position: 'atf',
          sizes: [10, 48, 15, 9, 14]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4641173'
        }
      }, {
        bidder: 'adform',
        params: {
          mid: 1634952,
          rcur: 'USD'
        }
      }, {
        bidder: 'omg-adform',
        params: {
          mid: 1658703,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'fragile-adform',
        params: {
          mid: 1655728,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'ix',
        params: {
          siteId: 878557
        }
      }, {
        bidder: 'teads',
        params: {
          placementId: 185867,
          pageId: 170977
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 27156774
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'r2b2',
        params: {
          d: 'emimino.cz',
          p: '300x600',
          m: 0,
          g: 'hb'
        }
      }]
    }, {
      code: 'halfpagead_spodni',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 600],
            [300, 300],
            [300, 250],
            [160, 600]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 144582,
          zoneId: 1139980,
          accountId: '10900',
          position: 'atf',
          sizes: [10, 48, 15, 9]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 144584,
          zoneId: 1139978,
          accountId: '10900',
          position: 'atf',
          sizes: [10, 48, 15, 9]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4641174'
        }
      }, {
        bidder: 'adform',
        params: {
          mid: 1634953,
          rcur: 'USD'
        }
      }, {
        bidder: 'omg-adform',
        params: {
          mid: 1658697,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'fragile-adform',
        params: {
          mid: 1655729,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'ix',
        params: {
          siteId: 878558
        }
      }, {
        bidder: 'teads',
        params: {
          placementId: 185867,
          pageId: 170977
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 27156776
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'r2b2',
        params: {
          d: 'emimino.cz',
          p: '300x600_spodni',
          m: 0,
          g: 'hb'
        }
      }]
    }, {
      code: 'm_300x250',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 250],
            [250, 250],
            [480, 480]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 157330,
          zoneId: 1145524,
          accountId: '10900',
          position: 'atf',
          sizes: [15, 14, 261]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 157332,
          zoneId: 1145522,
          accountId: '10900',
          position: 'atf',
          sizes: [15, 14, 261]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4641175'
        }
      }, {
        bidder: 'adform',
        params: {
          mid: 1634954,
          rcur: 'USD'
        }
      }, {
        bidder: 'omg-adform',
        params: {
          mid: 1658704,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'fragile-adform',
        params: {
          mid: 1655731,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'ix',
        params: {
          siteId: 878559
        }
      }, {
        bidder: 'teads',
        params: {
          placementId: 185867,
          pageId: 170977
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 27156777
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'r2b2',
        params: {
          d: 'emimino.cz',
          p: '300x250',
          m: 1,
          g: 'hb'
        }
      }]
    }, {
      code: 'm_300x250_clankovy',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 250],
            [250, 250],
            [480, 480]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 157330,
          zoneId: 1405424,
          accountId: '10900',
          position: 'atf',
          sizes: [15, 14, 261]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 157332,
          zoneId: 1405422,
          accountId: '10900',
          position: 'atf',
          sizes: [15, 14, 261]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4641176'
        }
      }, {
        bidder: 'adform',
        params: {
          mid: 1634955,
          rcur: 'USD'
        }
      }, {
        bidder: 'omg-adform',
        params: {
          mid: 1658701,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'fragile-adform',
        params: {
          mid: 1655732,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'ix',
        params: {
          siteId: 878560
        }
      }, {
        bidder: 'teads',
        params: {
          placementId: 185866,
          pageId: 170976
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 27156779
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'r2b2',
        params: {
          d: 'emimino.cz',
          p: '300x250_clankovy',
          m: 1,
          g: 'hb'
        }
      }]
    }, {
      code: 'm_branding',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 250],
            [250, 250],
            [480, 820],
            [300, 600],
            [768, 1230],
            [480, 480]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 157330,
          zoneId: 1148154,
          accountId: '10900',
          position: 'atf',
          sizes: [15, 14, 256, 10, 261]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 157332,
          zoneId: 1148296,
          accountId: '10900',
          position: 'atf',
          sizes: [15, 14, 256, 10, 261]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4641177'
        }
      }, {
        bidder: 'adform',
        params: {
          mid: 1634949,
          rcur: 'USD'
        }
      }, {
        bidder: 'omg-adform',
        params: {
          mid: 1658700,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'fragile-adform',
        params: {
          mid: 1655733,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'ix',
        params: {
          siteId: 844205
        }
      }, {
        bidder: 'teads',
        params: {
          placementId: 185866,
          pageId: 170976
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 27156780
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'r2b2',
        params: {
          d: 'emimino.cz',
          p: 'interscroller',
          m: 1,
          g: 'hb'
        }
      }]
    }, {
      code: 'm_paticka',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 250]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 157330,
          zoneId: 1405420,
          accountId: '10900',
          position: 'atf',
          sizes: [15]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 157332,
          zoneId: 1405418,
          accountId: '10900',
          position: 'atf',
          sizes: [15]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4641178'
        }
      }, {
        bidder: 'adform',
        params: {
          mid: 1634948,
          rcur: 'USD'
        }
      }, {
        bidder: 'omg-adform',
        params: {
          mid: 1658708,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'fragile-adform',
        params: {
          mid: 1655734,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'ix',
        params: {
          siteId: 878561
        }
      }, {
        bidder: 'teads',
        params: {
          placementId: 185867,
          pageId: 170977
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 27156781
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'r2b2',
        params: {
          d: 'emimino.cz',
          p: '300x250_paticka',
          m: 1,
          g: 'hb'
        }
      }]
    }, {
      code: 'm_sticky',
      mediaTypes: {
        banner: {
          sizes: [
            [320, 150],
            [300, 250],
            [500, 200]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 157330,
          zoneId: 1149070,
          accountId: '10900',
          position: 'atf',
          sizes: [60, 15, 258]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 157332,
          zoneId: 1149068,
          accountId: '10900',
          position: 'atf',
          sizes: [60, 15, 258]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'adform',
        params: {
          mid: 1634957,
          rcur: 'USD'
        }
      }, {
        bidder: 'omg-adform',
        params: {
          mid: 1658706,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'fragile-adform',
        params: {
          mid: 1655735,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'teads',
        params: {
          placementId: 185867,
          pageId: 170977
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 27156782
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }]
    }, {
      code: 'widesquare',
      mediaTypes: {
        banner: {
          sizes: [
            [480, 300],
            [300, 300],
            [250, 250],
            [300, 250],
            [480, 480]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 144582,
          zoneId: 1405410,
          accountId: '10900',
          position: 'atf',
          sizes: [83, 48, 14, 15, 261]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 144584,
          zoneId: 1405412,
          accountId: '10900',
          position: 'atf',
          sizes: [83, 48, 14, 15, 261]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4641179'
        }
      }, {
        bidder: 'adform',
        params: {
          mid: 1634946,
          rcur: 'USD'
        }
      }, {
        bidder: 'omg-adform',
        params: {
          mid: 1658705,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'fragile-adform',
        params: {
          mid: 1655737,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'ix',
        params: {
          siteId: 878562
        }
      }, {
        bidder: 'teads',
        params: {
          placementId: 185867,
          pageId: 170977
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 27156783
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'r2b2',
        params: {
          d: 'emimino.cz',
          p: '480x300',
          m: 0,
          g: 'hb'
        }
      }]
    }, {
      code: 'widesquare_spodni',
      mediaTypes: {
        banner: {
          sizes: [
            [480, 300],
            [300, 300],
            [250, 250],
            [300, 250],
            [480, 480]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 144582,
          zoneId: 1405414,
          accountId: '10900',
          position: 'atf',
          sizes: [83, 48, 14, 15, 261]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 144584,
          zoneId: 1405416,
          accountId: '10900',
          position: 'atf',
          sizes: [83, 48, 14, 15, 261]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4641180'
        }
      }, {
        bidder: 'adform',
        params: {
          mid: 1634947,
          rcur: 'USD'
        }
      }, {
        bidder: 'omg-adform',
        params: {
          mid: 1658696,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'fragile-adform',
        params: {
          mid: 1655726,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'ix',
        params: {
          siteId: 878563
        }
      }, {
        bidder: 'teads',
        params: {
          placementId: 185867,
          pageId: 170977
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 27156784
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'r2b2',
        params: {
          d: 'emimino.cz',
          p: '480x300_spodni',
          m: 0,
          g: 'hb'
        }
      }]
    }, {
      code: 'wallpaper',
      mediaTypes: {
        banner: {
          sizes: [
            [970, 310],
            [728, 90],
            [970, 250]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 144582,
          zoneId: 2639014,
          accountId: '10900',
          position: 'atf',
          sizes: [94, 2, 57]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 144584,
          zoneId: 2639018,
          accountId: '10900',
          position: 'atf',
          sizes: [94, 2, 57]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4783436'
        }
      }, {
        bidder: 'adform',
        params: {
          mid: 1634958,
          rcur: 'USD'
        }
      }, {
        bidder: 'omg-adform',
        params: {
          mid: 1658707,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'fragile-adform',
        params: {
          mid: 1655736,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'ix',
        params: {
          siteId: 907727
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 28018616
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'r2b2',
        params: {
          d: 'emimino.cz',
          p: '970x310',
          m: 0,
          g: 'hb'
        }
      }]
    }, {
      code: 'halfpagead_lazyload',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 600],
            [300, 300],
            [300, 250],
            [160, 600]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 144582,
          zoneId: 2639010,
          accountId: '10900',
          position: 'atf',
          sizes: [10, 48, 15, 9]
        }
      }, {
        bidder: 'rubicon-mask',
        params: {
          siteId: 144584,
          zoneId: 2639012,
          accountId: '10900',
          position: 'atf',
          sizes: [10, 48, 15, 9]
        },
        bidderModuleName: 'rubicon'
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4783434'
        }
      }, {
        bidder: 'adform',
        params: {
          mid: 1634959,
          rcur: 'USD'
        }
      }, {
        bidder: 'omg-adform',
        params: {
          mid: 1658698,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'fragile-adform',
        params: {
          mid: 1655730,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'ix',
        params: {
          siteId: 907726
        }
      }, {
        bidder: 'teads',
        params: {
          placementId: 185867,
          pageId: 170977
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 28018615
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'r2b2',
        params: {
          d: 'emimino.cz',
          p: '300x600_lazyload',
          m: 0,
          g: 'hb'
        }
      }]
    }, {
      code: 'widesquare_clankovy2',
      mediaTypes: {
        banner: {
          sizes: [
            [480, 300],
            [300, 300],
            [250, 250],
            [300, 250],
            [480, 480]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 144582,
          zoneId: 2766642,
          accountId: '10900',
          position: 'atf',
          sizes: [83, 48, 14, 15, 261]
        }
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '4945298'
        }
      }, {
        bidder: 'adform',
        params: {
          mid: 1643112,
          rcur: 'USD'
        }
      }, {
        bidder: 'omg-adform',
        params: {
          mid: 1658699,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'fragile-adform',
        params: {
          mid: 1655738,
          rcur: 'USD'
        },
        bidderModuleName: 'adform'
      }, {
        bidder: 'ix',
        params: {
          siteId: 958823
        }
      }, {
        bidder: 'teads',
        params: {
          placementId: 185866,
          pageId: 170976
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 29193933
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'r2b2',
        params: {
          d: 'emimino.cz',
          p: '480x300_clankovy_2',
          m: 0,
          g: 'hb'
        }
      }]
    }, {
      code: 'm_300x250_souvisejici',
      mediaTypes: {
        banner: {
          sizes: [
            [300, 250],
            [250, 250]
          ]
        }
      },
      bids: [{
        bidder: 'rubicon',
        params: {
          siteId: 157330,
          zoneId: 2964102,
          accountId: '10900',
          position: 'atf',
          sizes: [15, 14]
        }
      }, {
        bidder: 'pubmatic',
        params: {
          publisherId: '158732',
          adSlot: '5281631'
        }
      }, {
        bidder: 'adform',
        params: {
          mid: 1726650,
          rcur: 'USD'
        }
      }, {
        bidder: 'ix',
        params: {
          siteId: 1014827
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 30438855
        }
      }, {
        bidder: 'criteo',
        params: {
          networkId: 11172
        }
      }, {
        bidder: 'r2b2',
        params: {
          d: 'emimino.cz',
          p: '300x250_souvisejici',
          m: 1,
          g: 'hb'
        }
      }]
    }, {
      code: 'nativ',
      mediaTypes: {
        native: {}
      },
      bids: [{
        bidder: 'adform',
        params: {
          mid: 1782810,
          rcur: 'USD'
        }
      }, {
        bidder: 'appnexus',
        params: {
          placementId: 31269807
        }
      }]
    }]
  },
  formats: {
    interscroller: {
      enabled: true
    },
    slideup: {
      enabled: true,
      adUnits: ['m_sticky']
    },
    skin: {
      enabled: true,
      backgroundEl: /*S*/ document.getElementById('main') /*E*/ ,
      contentEl: /*S*/ document.getElementsByClassName('m-bg-4')[0] /*E*/ ,
      contentCSS: ' '
    },
    vignette: {
      enabled: true,
      adUnits: ['m_vignete']
    },
    native: {
      templates: {
        nativ: "<div class='advert-topic'>\n  <div class='advert-topic'>\n    <div class='advert-topic__content'>\n      <a rel='sponsored' href='${link}' target='_blank' class='color--default'><span>${title} - ${desc}</span></a>\n    </div>\n    <div class='advert-topic__image'>\n      <a rel='sponsored' href='${link}' target='_blank'>\n        <picture><img class='image-response' src='${img}'></picture>\n      </a>\n    </div>\n  </div>\n</div>",
        m_nativ: "<div class='advert-topic'>\n  <div class='advert-topic'>\n    <div class='advert-topic__content'>\n      <a rel='sponsored' href='${link}' target='_blank' class='color--default'><span>${title}</span></a>\n    </div>\n    <div class='advert-topic__image' style='width:50%'>\n      <a rel='sponsored' href='${link}' target='_blank'>\n        <picture><img class='image-response' src='${img}'></picture>\n      </a>\n    </div>\n  </div>\n</div>"
      },
      enabled: true
    }
  }
}